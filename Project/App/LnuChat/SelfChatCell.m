...
//  lnuchat
//
//  Created by Mikael Melander on 2016-04-03.
...
+ (id)CellForTableWidth:(CGFloat)width {
    ...
    SelfChatCell *cell = [[SelfChatCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:SCIdentifier];
    cell.alpha = 0;
    ...
    UIImageView *PhImg = [[UIImageView alloc] initWithFrame:CGRectMake(kTableWidth-kStandardSize-kPadding+3, kPadding, kStandardSize, kStandardSize)];
   ...
   // marker.layer.cornerRadius = 2;
    ...
    UILabel *HeaderLabel = [[UILabel alloc] initWithFrame:CGRectMake(kTableWidth-kStandardSize+kPadding*2, kPadding, kTableWidth-(PhImg.frame.size.width+PhImg.frame.origin.x+kPadding*2.5), kStandardLabelHeight)];
    HeaderLabel.font = k_smallfont;
    cell.Hlabel = HeaderLabel;
    [cell addSubview:cell.Hlabel];
    
    UIImageView *bkImg = [[UIImageView alloc] initWithFrame:CGRectMake(kTableWidth-kStandardSize+(kPadding*2)-3, kPadding-3, kTableWidth-(kPadding*3)-kStandardSize+6, 57.5f)];
    bkImg.layer.cornerRadius = 5;
    cell.bkImage = bkImg;
    [cell addSubview:cell.bkImage];
    
    UILabel *DetailedLabel = [[UILabel alloc] initWithFrame:CGRectMake(kTableWidth-kStandardSize+kPadding*2, kPadding+kStandardLabelHeight, kTableWidth-(kPadding*3)-kStandardSize, 57.5f)];
...
+ (CGFloat)cellHeight:(PFObject *)obj {
    ...
    CGSize labelSize = [SelfChatCell LabelSize:obj[@"Message"]];
...
- (void)configureTextForCell:(PFObject *)obj {
    ...
    CGSize labelSize = [SelfChatCell LabelSize:message];
    CGFloat labelHeight = labelSize.height;
    CGFloat result = labelHeight+(kPadding*2)+kStandardLabelHeight+25;
    if (result < 70) {
        result = 70;
    ...
    [self.Hlabel sizeToFit];
    self.Hlabel.frame = CGRectMake(self.PlaceholderImg.frame.origin.x-self.Hlabel.frame.size.width-(kPadding), kPadding, self.Hlabel.frame.size.width, self.Hlabel.frame.size.height);
    ...
    self.Dlabel.frame = CGRectMake(kTableWidth-(kStandardSize+kPadding*2)-labelSize.width, kPadding+kStandardLabelHeight+5, kTableWidth-(kPadding*3)-kStandardSize, labelHeight);
